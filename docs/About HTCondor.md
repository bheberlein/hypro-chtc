...
The **Schedd** (scheduler) daemon is 
...
#### Input Files
 ...
 - any other files, directories etc that HTCondor is to transfer to the remote scratch directory, to set up the execution environment for the job before it is run. This should be given as a comma-separated list to the `transfer_input_files` command.  
