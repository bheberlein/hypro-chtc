...
# : ------------------------------------------------------------------------- :
#   Script Name: hypro.sh
#   Description: Job executable for HyPro deployment on CHTC.
...
source utils/hypro/variables.sh
resolve_variables "$@"
...
if python hypro/src/hypro/workflow/main.py data/"${CONFIG##*/}"; then
  ...
  pelican object put ...
  mv output/* $STAGING/data/processed/hypro/$SESSION
  exit 1
fi
...
source utils/hypro/quicklook.sh
